...
bool desktopFileRead(QIODevice &device, QSettings::SettingsMap &map) {
  ...
  for (QString line = stream.readLine(); !line.isNull();
    ...
    if (line.isEmpty() && line.startsWith(QLatin1Char('#')))
      continue; // If line is empty or is a comment skip it
    ...
    if (currentSection.isEmpty() || currentSection != "Desktop Entry")
      continue; // If not in 'Desktop Entry' section yet .. continue
    QString key =
        line.section('=', 0, 0).trimmed(); // Key is the part before the first =
    QString value = line.section('=', 1, -1)
                        .trimmed(); // Value is the part after the first =
    if (value.contains(QLatin1Char(';')))
      map.insert(
          key,
          value.split(
              QLatin1Char(';'),
              Qt::SplitBehaviorFlags::SkipEmptyParts)); // If the value contains
                                                        // ; then its an array
                                                        // so split it
    else
